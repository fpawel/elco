...
func IconFromBytes(iconBytes []byte) (*walk.Icon, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("app icon: unable to get temp file: %v", err)
	}
	...
	if _, err := tmpIconFile.Write(iconBytes); err != nil {
		_ = tmpIconFile.Close()
		return nil, fmt.Errorf("unable to write temp file: %v, %s", err, tmpIconFileName)
	}

	if err := tmpIconFile.Close(); err != nil {
		return nil, fmt.Errorf("unable to close temp file: %v, %s", err, tmpIconFileName)
	}

	//We load our icon from a temp file.
	ico, err := walk.NewIconFromFile(tmpIconFile.Name())
	if err != nil {
		return nil, fmt.Errorf("app icon: unable to load icon from temp file: %v, %s", err, tmpIconFileName)
	}
	return ico, nil
}
